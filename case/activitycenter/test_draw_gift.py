...
@ddt
class testDrawgift(unittest.TestCase):

    @classmethod
    def setUpClass(cls) -> None:
        cls.log = Log()
        cls.url = get_host('test') + '/admin/appGiftAndActivity/applyGift'
        cls.data = "{\n \"frontType\": \"ANDROID\", \"giftId\": \"\",\"subtabulationId\":\"\" \n}"
        cls.headers = {'Content-Type': 'application/json', 'authorization': 'bearer 4cb6a59d-3e16-493f-922e-5146ca58f87d'}
        cls.method = 'post'
    
    @data(*([0, '']))
    def test_draw_gift(self, test_data):
        '''
        测试获取领取礼包接口
        :param:
        :return:
