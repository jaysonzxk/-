...
@ddt
class testUserdayrewards(unittest.TestCase):

    @classmethod
    def setUpClass(cls) -> None:
        cls.log = Log()
        cls.url = get_host('test') + '/admin/rewardsBillReport/getUserRewardsBillPage'
        cls.data = "{\"pageParam\":{\"current\":1,\"orderBy\":\"rb.create_time\",\"orderType\":\"desc\",\"size\":15},\n\"searchParam\":{\"startTime\":\"\",\"endTime\":\"\"}}"
        cls.headers = {'Content-Type': 'application/json', 'authorization': 'bearer 4cb6a59d-3e16-493f-922e-5146ca58f87d'}
        cls.method = 'post'
    
    @data(*([0, '']))
    def test_user_day_rewards(self, test_data):
        '''
        测试获取获取当日活动礼金接口
        :param:
        :return:
        ...
        if self.method == 'post':
            if self.headers.get('Content-Type') == 'application/json;charset=UTF-8' and self.data != 'nan':
                ...
                response = [results['code'], results['msg']]
            self.log.info('----------测试开始----------')
            self.log.info('测试场景：[获取当日活动礼金]')
            self.log.info("测试断言-->期望值/校验值[[0, '']]")
            self.log.info('测试断言-->实际值[%s]' % response)
            self.log.info('请求参数:"{\"pageParam\":{\"current\":1,\"orderBy\":\"rb.create_time\",\"orderType\":\"desc\",\"size\":15},\n\"searchParam\":{\"startTime\":\"\",\"endTime\":\"\"}}"')
